...
def worker_setup_node():
    while True:
        ...
        if node.user != None and node.passwd != None:
            ...
            Helper.copy_pkg_scripts_to_remote_with_passwd(node)
            # deploy node
...
def deploy_by_bcf_config(config):
    ...
    node_dic = Helper.load_bcf_config(config, env)
...
if __name__=='__main__':
    ...
    parser.add_argument('-f', "--fuel-config-file", required=False,
                        help="YAML file that is generated by Fuel, will override BCF YAML file's node configuration.")
